...
import bungeeCordService from './bungeecord';
...
class WaterfallService {
    ...
    private async readWaterfallConfig(): Promise<WaterfallConfig> {
        try {
            const configBuffer = await webdavService.getFileContents(this.waterfallConfigPath);
            const configContent = configBuffer.toString('utf-8');
            return this.parseWaterfallConfig(configContent);
        } catch {
     ...
     */
    private parseWaterfallConfig(content: string): WaterfallConfig {
     ...
     */
    private serializeBungeeCordConfig(config: any): string {
        ...
        for (const [name, server] of Object.entries(config.servers)) {
            yaml += `  ${name}:\n`;
            yaml += `    motd: '${(server as any).motd}'\n`;
            yaml += `    address: ${(server as any).address}\n`;
            yaml += `    restricted: ${(server as any).restricted}\n`;
        }
